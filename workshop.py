...
def create_workshop_ou(name, scp_template, account_ids, cfn_template):
    ...
    create_organizationl_unit_response = organizations_client.create_organizational_unit(
        ParentId=root_id, Name=name
    )
    ...
    organizations_client.attach_policy(PolicyId=policy_id, TargetId=ou_id)
...
def remove_workshop_ou(name):
    ...
    ous = organizations_client.list_organizational_units_for_parent(ParentId=root_id)[
        "OrganizationalUnits"
    ]
    ...
    policies = organizations_client.list_policies_for_target(
...
def move_account(account_id, target_ou_id):
    ...
    try:
        ...
        organizations_client.move_account(
            AccountId=account_id, SourceParentId=parent_id, DestinationParentId=target_ou_id
        )
...
def setup_account(account_id, account_alias, cfn_baseline_template):
    ...
    iam_client.attach_user_policy(
        UserName=iam_user_name, PolicyArn="arn:aws:iam::aws:policy/AdministratorAccess"
    )
    iam_client.create_login_profile(
        UserName=iam_user_name, Password=iam_user_password, PasswordResetRequired=False
    )
    ...
    baseline_stack_output = cfn_client.describe_stacks(StackName=cfn_stackname)["Stacks"][0][
        "Outputs"
    ]
    return {
...
parser.add_argument(
    "--action", metavar="action", help="operation to perform for workshop management", required=True
)
parser.add_argument("--ou-name", metavar="ou_name", help="name of AWS Organizational Unit")
parser.add_argument(
    "--account-ids", nargs="+", metavar="account_ids", type=str, help="the list of AWS Account ID's"
)
parser.add_argument(
    "--account-name",
    metavar="account_name",
    type=str,
    help="AWS Account Name for ACCOUNT_CREATE action",
)
parser.add_argument(
    "--account-email",
    metavar="account_email",
    type=str,
    help="AWS Account Email for ACCOUNT_CREATE action",
)
